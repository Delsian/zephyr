...
static int spi_flash_at45_init(const struct device *dev)
{
	...
	if (dev_config->reset_gpio) {
		const struct device *reset_gpio = device_get_binding(
					dev_config->reset_gpio);
		if (!reset_gpio) {
			LOG_ERR("Cannot find %s", dev_config->reset_gpio);
			return -EPERM;
		}

		if (gpio_pin_configure(reset_gpio, dev_config->reset_pin,
								dev_config->reset_dt_flags)) {
			LOG_ERR("Couldn't configure reset pin");
			return -EIO;
		}
		gpio_pin_set(reset_gpio, dev_config->reset_pin, 0);
	}
...
#define SPI_FLASH_AT45_INST(idx)					     \
	...
	static const struct spi_flash_at45_config inst_##idx##_config = {    \
		...
		IF_ENABLED(DT_INST_NODE_HAS_PROP(idx, reset_gpios), (		     \
			.reset_gpio = DT_INST_GPIO_LABEL(idx, reset_gpios),      \
