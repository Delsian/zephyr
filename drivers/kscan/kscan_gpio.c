...
const struct device *rows[MATRIX_ROWS];
const struct device *cols[MATRIX_COLS];
...
static int kscan_gpio_enable_interrupts(const struct device *dev)
{
	const struct kscan_gpio_config *cfg = dev->config;

	return kscan_gpio_config_interrupts(
			cols, cfg->cols, MATRIX_COLS,
			GPIO_INT_DEBOUNCE | GPIO_INT_EDGE_BOTH);

}

static int kscan_gpio_disable_interrupts(const struct device *dev)
{
	const struct kscan_gpio_config *cfg = dev->config;
...
static void kscan_gpio_set_row_state(const struct device *dev, int value)
{
	const struct kscan_gpio_config *cfg = dev->config;

	for (int r = 0; r < MATRIX_ROWS; r++) {
		const struct device *row = rows[r];
...
static int kscan_gpio_read(const struct device *dev)
{
	...
	for (int r = 0; r < MATRIX_ROWS; r++) {
		...
		for (int c = 0; c < MATRIX_COLS; c++) {
			const struct device *col = cols[c];
...
static int kscan_gpio_enable_interrupts(const struct device *dev)
{
	...
	return kscan_gpio_config_interrupts(rows, cfg->rows, MATRIX_ROWS,
			GPIO_INT_DEBOUNCE | GPIO_INT_EDGE_BOTH);
}
...
static void kscan_gpio_set_column_state(const struct device *dev, int value)
{
	const struct kscan_gpio_config *cfg = dev->config;
...
static void kscan_gpio_irq_callback_handler(const struct device *dev,
...
{
	kscan_gpio_read(kscan_dev);
}

static int kscan_gpio_configure(const struct device *dev, kscan_callback_t callback)
{
	struct kscan_gpio_data *data = dev->data;
...
static int kscan_gpio_init(const struct device *dev)
{
	struct kscan_gpio_data *data = dev->data;
	const struct kscan_gpio_config *cfg = dev->config;
	int err;
	...
	for (int c = 0; c < MATRIX_COLS; c++) {
		...
		cols[c] = device_get_binding(item_cfg->label);
		if (cols[c] == NULL) {
			LOG_ERR("Unable to find column GPIO device\n");
			return -EINVAL;
		}

		err = gpio_pin_configure(cols[c],
				item_cfg->pin,
		...
		gpio_init_callback(&irq_callbacks[c].callback,
				...
				BIT(item_cfg->pin));
		gpio_add_callback(cols[c], &irq_callbacks[c].callback);
#endif
	}
	...
	for (int r = 0; r < MATRIX_ROWS; r++) {
		...
		rows[r] = device_get_binding(item_cfg->label);
		if (rows[r] == NULL) {
			LOG_ERR("Unable to find row GPIO device\n");
			return -EINVAL;
		}

		int err = gpio_pin_configure(rows[r],
				item_cfg->pin,
		...
		gpio_init_callback(&irq_callbacks[r].callback,
				kscan_gpio_irq_callback_handler,
				BIT(item_cfg->pin));
		gpio_add_callback(rows[r], &irq_callbacks[r].callback);
#endif
	}

	kscan_dev = dev;
	data->callback = NULL;
	/* Initialize matrix_state with default values */
	kscan_gpio_read(dev);
...
DEVICE_DT_INST_DEFINE(0, kscan_gpio_init, device_pm_control_nop,
